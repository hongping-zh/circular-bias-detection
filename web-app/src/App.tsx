...
import { generateJossReview, generatePrFollowupAdvice } from './services/geminiService';
    ...
    <div className="fade-in">
      <section className="bg-white dark:bg-slate-800/50 p-8 rounded-xl shadow-lg">
        ...
        <div className="text-center mb-6">
            <a href="https://joss.theoj.org/about#submission_requirements" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 hover:underline transition-colors">
                <ExternalLinkIcon className="w-4 h-4" />
                <span>View Official JOSS Submission Guidelines</span>
            </a>
        </div>
...
const PrHelper: React.FC = () => {
    ...
    return (
      <div className="fade-in">
        ...
        <section className="mt-12">
            {isLoading && (<div className="flex flex-col items-center justify-center text-center text-slate-500 dark:text-slate-400"><SpinnerIcon className="w-12 h-12 mb-4" /><p className="text-lg font-semibold">Generating advice...</p></div>)}
            {error && (<div className="max-w-2xl mx-auto text-center bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-600 text-red-700 dark:text-red-300 px-4 py-3 rounded-lg" role="alert"><strong className="font-bold">Error: </strong><span className="block sm:inline">{error}</span></div>)}
            {advice && (
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans">
      ...
      <main className="container mx-auto px-4 py-10 sm:py-16 max-w-5xl">
        ...
        {activeTab === 'joss' ? <JossReviewer /> : <PrHelper />}
      </main>

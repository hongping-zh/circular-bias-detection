# Zenodo Auto-Sync Guide (GitHub â†?Zenodo)
...
1. Go to Zenodo: Account â†?GitHub
...
1. In GitHub â†?Releases â†?Draft a new release
...
Example (Concept DOI â€?replace with your concept DOI once available):
...
- Reuse the Version DOI in papersâ€?â€œCode/Data availabilityâ€?sections for reproducibility
...
- CI failing on version mismatch? Ensure `CITATION.cff: version` = tag without `v` (e.g., tag `v0.1.0` â†?version `0.1.0`)
